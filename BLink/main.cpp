...
public:
    ...
    void operator()() {
        ...
        for (std::size_t i = 0; i < NUMBER_OPERATIONS; ++i) {
            std::size_t random = distribution(rd);
            std::cout << i << ": " << random << "  ";
            std::cout << b_link_->search(random) << "\n";;
        }
    }
...
void sequential_insert(EDA::Concurrent::BLinkTree<3, int>* b_link) {
    ...
    for (std::size_t i = 0; i < NUMBER_OPERATIONS; ++i) {
        std::size_t random = distribution(rd);
        std::cout << random << "\n";
        std::cout << "NUMERO DE OPERACION: " << i+1 << std::endl;
        b_link->insert(random);
        b_link->print_endl();
    }
}
